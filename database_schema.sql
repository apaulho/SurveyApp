...
CREATE TABLE surveydb (
    ...
    created_by_user_id INTEGER REFERENCES userdb(user_id) NOT NULL,
    is_active BOOLEAN DEFAULT true,
...
-- 4. SurveyQuestionDB Table
-- Links surveys to questions and defines question order within surveys
CREATE TABLE surveyquestiondb (
    survey_question_id SERIAL PRIMARY KEY,
    survey_id INTEGER REFERENCES surveydb(survey_id) ON DELETE CASCADE,
...
CREATE INDEX idx_surveydb_company ON surveydb(company_id);
CREATE INDEX idx_surveydb_created_by ON surveydb(created_by_user_id);
CREATE INDEX idx_surveydb_active ON surveydb(is_active);
CREATE INDEX idx_surveyquestiondb_survey ON surveyquestiondb(survey_id);
CREATE INDEX idx_surveyquestiondb_question ON surveyquestiondb(question_id);
...
CREATE TRIGGER update_surveydb_updated_at BEFORE UPDATE ON surveydb FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
